...
test.describe.skip("Setmore Booking ", () => {
  ...
  test.slow(
    "To check the teleportlink and booking appointment created",
    async () => {
...
test.describe("To verify all the apps in integration page", () => {
  test.beforeAll(async ({ browser }) => {
    page = await browser.newPage();
    loginapp = new LoginPage(page);
    selectDate = new Selectdate(page);
  ...
  test.only("Toverify all apps are present in webpage", async () => {
